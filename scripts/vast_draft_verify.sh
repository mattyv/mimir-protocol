#!/usr/bin/env bash
# Stage-3b-i draft-and-verify on a Vast RTX 3090 (run_draft_verify.py): with
# REAL thoughts, does checking K drafts beat greedy, and do the picks move the
# reasoning FORWARD? Loads the Stage-1 gist adapter, per step samples K
# candidate next-steps from the injected thought, verifies each by likelihood
# under the real reasoning-so-far, picks best. Eval-only, NO push (~$0.50-1,
# ~40-90 min). All setup hardening reused from vast_mimir.sh.
#
#   HF_TOKEN=... ./scripts/vast_draft_verify.sh
set -euo pipefail
...
echo "=== run: draft-verify 3b-i (ndocs=${NDOCS} k=${KDRAFTS} temp=${TEMP}) ==="
timeout ${TIMEOUT} env PYTHONPATH=src python -u -m marker.run_draft_verify \
  --model-name ${MODEL} --repo ${REPO} --dataset ${DATASET} --unit ${UNIT} \
  --n-docs ${NDOCS} --k-drafts ${KDRAFTS} --temperature ${TEMP} 2>&1 | tee /root/dv.log
kill \$HB 2>/dev/null
echo "DV_RC=\${PIPESTATUS[0]}" | tee -a /root/dv.log
echo "ALLDONE" | tee -a /root/dv.log
EOS
...
echo "→ poller:  bash scripts/vast_poll_destroy.sh $INSTANCE_ID 140"
