...
# run_instruct_skills on 7B base + 7B-Instruct via onstart (no SSH — this
...
PYTHONPATH=src python -m marker.run_instruct_skills \
  ...
  --max-new 200 2>&1 | tee /root/skills.log
echo "EXITRC=${PIPESTATUS[0]}" | tee -a /root/skills.log
echo "ALLDONE" | tee -a /root/skills.log
EOS
