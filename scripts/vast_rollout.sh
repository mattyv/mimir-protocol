#!/usr/bin/env bash
# Latent chain rollout on a Vast RTX 3090 (~$1.50): the real fast-lane test.
# Loads the trained encoder + predictor + bridge (no training), fixes a real
# PREFIX of thoughts per held-out doc, then free-runs the predictor on its own
# outputs and injects each predicted thought to score how well it predicts the
# true next step. Reports the DRIFT curve (free vs teacher-forced gap_closed by
# rollout depth) — headline: the depth where free-running stops beating the
# shuffled floor.
#
# Defaults match the predictor's training distribution: open-r1 sentence-unit,
# window=6, skip first 2000 docs (its train/eval range).
#
#   HF_TOKEN=... ./scripts/vast_rollout.sh
#   NDOCS=600 PREFIX=2 MAXDEPTH=12 HF_TOKEN=... ./scripts/vast_rollout.sh
set -euo pipefail
...
PREFIX="${PREFIX:-2}"                            # real thoughts before free-running
...
echo "=== ROLLOUT (subdir=${SUBDIR} dataset=${DATASET} unit=${UNIT} ndocs=${NDOCS} skip=${SKIPDOCS} window=${WINDOW} prefix=${PREFIX} max_depth=${MAXDEPTH}) ==="
timeout ${TIMEOUT} env PYTHONPATH=src python -u -m marker.run_rollout \
  ...
  --prefix ${PREFIX} --max-depth ${MAXDEPTH} 2>&1 | tee /root/rollout.log
echo "ROLLOUT_RC=\${PIPESTATUS[0]}" | tee -a /root/rollout.log
kill \$HB 2>/dev/null
echo "ALLDONE" | tee -a /root/rollout.log
EOS
