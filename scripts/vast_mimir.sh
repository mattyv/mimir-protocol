#!/usr/bin/env bash
# Stage-3 3a-i ceiling test on a Vast RTX 3090 (run_mimir_decode.py): does a
# REAL thought decode back to its next step? Loads the Stage-1 gist adapter,
# encodes held-out reasoning steps, injects each thought's full per-layer KV,
# and reports the soft PPL ceiling + the hard greedy-generate F1. Eval-only,
# NO push (~$0.50, ~20-40 min). All setup hardening reused from vast_cot.sh.
#
#   HF_TOKEN=... ./scripts/vast_mimir.sh
set -euo pipefail
...
echo "=== run: mimir 3a-i ceiling (dataset=${DATASET} unit=${UNIT} ndocs=${NDOCS}) ==="
timeout ${TIMEOUT} env PYTHONPATH=src python -u -m marker.run_mimir_decode \
  --model-name ${MODEL} --repo ${REPO} --dataset ${DATASET} --unit ${UNIT} --n-docs ${NDOCS} 2>&1 | tee /root/mimir.log
kill \$HB 2>/dev/null
echo "MIMIR_RC=\${PIPESTATUS[0]}" | tee -a /root/mimir.log
echo "ALLDONE" | tee -a /root/mimir.log
EOS
...
echo "→ poller:  bash scripts/vast_poll_destroy.sh $INSTANCE_ID 90"
