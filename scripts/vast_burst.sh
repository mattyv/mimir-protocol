#!/usr/bin/env bash
# Anchored-burst end-to-end test on a Vast RTX 3090 (~$2): the fast-lane speed
# test. Loads encoder + predictor + frozen bridge (no training), generates GSM8K
# solutions under 4 arms (plain / burst_true / burst_pred / burst_none),
...
#
#   HF_TOKEN=... ./scripts/vast_burst.sh
#   NPROB=300 ANCHOR=2 HF_TOKEN=... ./scripts/vast_burst.sh
set -euo pipefail
...
NPROB="${NPROB:-200}"
ANCHOR="${ANCHOR:-2}"                           # decode 1 step, inject 1 (most aggressive burst)
WINDOW="${WINDOW:-8}"
...
echo "=== BURST (subdir=${SUBDIR} bridge=${BRIDGESUB} dataset=${DATASET} nprob=${NPROB} anchor_every=${ANCHOR} window=${WINDOW}) ==="
timeout ${TIMEOUT} env PYTHONPATH=src python -u -m marker.run_burst \
  ...
  --n-problems ${NPROB} --anchor-every ${ANCHOR} 2>&1 | tee /root/burst.log
echo "BURST_RC=\${PIPESTATUS[0]}" | tee -a /root/burst.log
kill \$HB 2>/dev/null
echo "ALLDONE" | tee -a /root/burst.log
EOS
