#!/usr/bin/env bash
# Anchored-gistprobe end-to-end test on a Vast RTX 3090 (~$2): the fast-lane speed
# test. Loads encoder + predictor + frozen bridge (no training), generates GSM8K
# solutions under 4 arms (plain / gistprobe_true / gistprobe_pred / gistprobe_none),
...
#
#   HF_TOKEN=... ./scripts/vast_gistprobe.sh
#   NPROB=300 ANCHOR=2 HF_TOKEN=... ./scripts/vast_gistprobe.sh
set -euo pipefail

IMAGE="pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel"
DISK_GB=90
MODEL="${MODEL:-Qwen/Qwen2.5-7B}"
REPO="${REPO:-mattyvee/mimir-artifacts}"        # Stage-1 adapter + render reader + push
RENDERSUB="${RENDERSUB:-render_adapter_ledger}"
DATASET="${DATASET:-openai/gsm8k}"
NPROB="${NPROB:-63}"                             # problems per set (hard/easy)
TIMEOUT="${TIMEOUT:-90m}"
...
echo "=== GISTPROBE (dataset=${DATASET} nprob=${NPROB}/set render=${RENDERSUB}) ==="
timeout ${TIMEOUT} env PYTHONPATH=src python -u -m marker.run_gistprobe \
  --model-name ${MODEL} --repo ${REPO} \
  --artifacts-repo ${REPO} --render-subdir ${RENDERSUB} --out-repo ${REPO} \
  --dataset ${DATASET} --n-problems ${NPROB} 2>&1 | tee /root/gistprobe.log
echo "GISTPROBE_RC=\${PIPESTATUS[0]}" | tee -a /root/gistprobe.log
kill \$HB 2>/dev/null
echo "ALLDONE" | tee -a /root/gistprobe.log
EOS
