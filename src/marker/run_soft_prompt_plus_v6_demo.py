"""Soft prompt+ v6 — v5 + batched training.

Each training step processes `batch_size` Q+A pairs in a single
forward+backward (padded to common length). GPU is generally
under-utilized at batch=1 for a 32B model; batching gives 2-4× more
useful gradient per wall-clock second.

Combined with H100 (set in the modal entrypoint), expect roughly
4-6× faster training than v5 on A100.

CLI accepts same flags as v5 + `--batch-size`. The v5 demo's data
...
from marker.run_soft_prompt_plus_v5_demo import (
    _V5_BOUNDARY_KEEP_DEFAULT,
    _build_training_set_v5,
)
...
def main() -> None:
    ...
    parser.add_argument("--n-steps", type=int, default=2000)  # cut from 3500 — v5 converged by ~2k
    ...
    print(
        ...
        f"boundary_keep={args.boundary_keep}\n"
    )
    ...
    for axiom in TEST_AXIOMS:
        ...
        print(
            f"trained sp+ v6 in {time.time() - t0:.1f}s. "
        ...
        def run_probe_set(label: str, probes: list[str]) -> None:
            ...
            for probe in probes:
                ...
                print(f"    [T+ sp+v6]:      {out_T.replace(chr(10), ' ').strip()[:280]}")
